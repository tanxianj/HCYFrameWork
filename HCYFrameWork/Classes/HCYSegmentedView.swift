...
public let HCYSegmentedViewAutomaticDimension: CGFloat = -1
...
/// - click: 通过点击item选中
/// - scroll: 通过滚动到item选中
public enum HCYSegmentedViewItemSelectedType {
    case unknown
    case code
    case click
    case scroll
...
public protocol HCYSegmentedViewListContainer {
...
public protocol HCYSegmentedViewDataSource: AnyObject {
    ...
    /// 返回数据源数组，数组元素必须是HCYSegmentedBaseItemModel及其子类
    ///
    /// - Parameter segmentedView: HCYSegmentedView
    /// - Returns: 数据源数组
    func itemDataSource(in segmentedView: HCYSegmentedView) -> [HCYSegmentedBaseItemModel]

    /// 返回index对应item的宽度，等同于cell的宽度。
    ///
    /// - Parameters:
    ///   - segmentedView: HCYSegmentedView
    ///   - index: 目标index
    /// - Returns: item的宽度
    func segmentedView(_ segmentedView: HCYSegmentedView, widthForItemAt index: Int) -> CGFloat
    ...
    ///   - segmentedView: HCYSegmentedView
    ///   - index: 目标index
    func segmentedView(_ segmentedView: HCYSegmentedView, widthForItemContentAt index: Int) -> CGFloat

    /// 注册cell class
    ///
    /// - Parameter segmentedView: HCYSegmentedView
    func registerCellClass(in segmentedView: HCYSegmentedView)
    ...
    ///   - segmentedView: HCYSegmentedView
    ///   - index: 目标index
    /// - Returns: HCYSegmentedBaseCell及其子类
    func segmentedView(_ segmentedView: HCYSegmentedView, cellForItemAt index: Int) -> HCYSegmentedBaseCell

    /// 根据当前选中的selectedIndex，刷新目标index的itemModel
    ///
    /// - Parameters:
    ///   - itemModel: HCYSegmentedBaseItemModel
    ///   - index: 目标index
    ///   - selectedIndex: 当前选中的index
    func refreshItemModel(_ segmentedView: HCYSegmentedView, _ itemModel: HCYSegmentedBaseItemModel, at index: Int, selectedIndex: Int)
    ...
    ///   - selectedType: 选中的类型
    func refreshItemModel(_ segmentedView: HCYSegmentedView, currentSelectedItemModel: HCYSegmentedBaseItemModel, willSelectedItemModel: HCYSegmentedBaseItemModel, selectedType: HCYSegmentedViewItemSelectedType)
    ...
    ///   - percent: 从左到右的百分比
    func refreshItemModel(_ segmentedView: HCYSegmentedView, leftItemModel: HCYSegmentedBaseItemModel, rightItemModel: HCYSegmentedBaseItemModel, percent: CGFloat)
}

/// 为什么会把选中代理分为三个，因为有时候只关心点击选中的，有时候只关心滚动选中的，有时候只关心选中。所以具体情况，使用对应方法。
public protocol HCYSegmentedViewDelegate: AnyObject {
    ...
    ///   - segmentedView: HCYSegmentedView
    ///   - index: 选中的index
    func segmentedView(_ segmentedView: HCYSegmentedView, didClickSelectedItemAt index: Int)
    ...
    ///   - segmentedView: HCYSegmentedView
    ///   - index: 选中的index
    func segmentedView(_ segmentedView: HCYSegmentedView, didScrollSelectedItemAt index: Int)
    ...
    ///   - percent: 从左往右计算的百分比
    func segmentedView(_ segmentedView: HCYSegmentedView, scrollingFrom leftIndex: Int, to rightIndex: Int, percent: CGFloat)
    ...
    ///   - segmentedView: HCYSegmentedView
    ///   - index: 目标index
    func segmentedView(_ segmentedView: HCYSegmentedView, canClickItemAt index: Int) -> Bool
}

/// 提供HCYSegmentedViewDelegate的默认实现，这样对于遵从HCYSegmentedViewDelegate的类来说，所有代理方法都是可选实现的。
public extension HCYSegmentedViewDelegate {
    func segmentedView(_ segmentedView: HCYSegmentedView, didSelectedItemAt index: Int) { }
    func segmentedView(_ segmentedView: HCYSegmentedView, didClickSelectedItemAt index: Int) { }
    func segmentedView(_ segmentedView: HCYSegmentedView, didScrollSelectedItemAt index: Int) { }
    func segmentedView(_ segmentedView: HCYSegmentedView, scrollingFrom leftIndex: Int, to rightIndex: Int, percent: CGFloat) { }
    func segmentedView(_ segmentedView: HCYSegmentedView, canClickItemAt index: Int) -> Bool { return true }
}

/// 内部会自己找到父UIViewController，然后将其automaticallyAdjustsScrollViewInsets设置为false，这一点请知晓。
open class HCYSegmentedView: UIView, HCYSegmentedViewRTLCompatible {
    open weak var dataSource: HCYSegmentedViewDataSource? {
        didSet {
            dataSource?.reloadData(selectedIndex: selectedIndex)
        }
    }
    open weak var delegate: HCYSegmentedViewDelegate?
    open private(set) var collectionView: HCYSegmentedCollectionView!
    ...
    public var listContainer: HCYSegmentedViewListContainer? = nil {
        didSet {
            listContainer?.defaultSelectedIndex = defaultSelectedIndex
            contentScrollView = listContainer?.contentScrollView()
        }
    }
    /// indicators的元素必须是遵从HCYSegmentedIndicatorProtocol协议的UIView及其子类
    ...
    /// 整体内容的左边距，默认HCYSegmentedViewAutomaticDimension（等于itemSpacing）
    open var contentEdgeInsetLeft: CGFloat = HCYSegmentedViewAutomaticDimension
    /// 整体内容的右边距，默认HCYSegmentedViewAutomaticDimension（等于itemSpacing）
    open var contentEdgeInsetRight: CGFloat = HCYSegmentedViewAutomaticDimension
    /// 点击切换的时候，contentScrollView的切换是否需要动画
    open var isContentScrollViewClickTransitionAnimationEnabled: Bool = true

    private var itemDataSource = [HCYSegmentedBaseItemModel]()
    ...
    private func commonInit() {
        ...
        collectionView = HCYSegmentedCollectionView(frame: CGRect.zero, collectionViewLayout: layout)
    ...
    open override func layoutSubviews() {
        ...
        //部分使用者为了适配不同的手机屏幕尺寸，HCYSegmentedView的宽高比要求保持一样。所以它的高度就会因为不同宽度的屏幕而不一样。计算出来的高度，有时候会是位数很长的浮点数，如果把这个高度设置给UICollectionView就会触发内部的一个错误。所以，为了规避这个问题，在这里对高度统一向下取整。
        //如果向下取整导致了你的页面异常，请自己重新设置HCYSegmentedView的高度，保证为整数即可。
    ...
    //MARK: - Public
    public final func dequeueReusableCell(withReuseIdentifier identifier: String, at index: Int) -> HCYSegmentedBaseCell {
        let indexPath = IndexPath(item: index, section: 0)
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: identifier, for: indexPath)
        guard cell.isKind(of: HCYSegmentedBaseCell.self) else {
            fatalError("Cell class must be subclass of HCYSegmentedBaseCell")
        }
        return cell as! HCYSegmentedBaseCell
    }
    ...
    open func reloadDataWithoutListContainer() {
        ...
        if dataSource?.isItemSpacingAverageEnabled == true && totalContentWidth < bounds.size.width {
            ...
            if contentEdgeInsetLeft == HCYSegmentedViewAutomaticDimension {
                itemSpacingCount += 1
            }else {
                totalItemSpacingWidth -= contentEdgeInsetLeft
            }
            if contentEdgeInsetRight == HCYSegmentedViewAutomaticDimension {
                itemSpacingCount += 1
            }else {
        ...
        if contentScrollView != nil {
            if contentScrollView!.frame.equalTo(CGRect.zero) &&
                ...
                //某些情况系统会出现HCYSegmentedView先布局，contentScrollView后布局。就会导致下面指定defaultSelectedIndex失效，所以发现contentScrollView的frame为zero时，强行触发其父视图链里面已经有frame的一个父视图的layoutSubviews方法。
                //比如HCYSegmentedListContainerView会将contentScrollView包裹起来使用，该情况需要HCYSegmentedListContainerView.superView触发布局更新
        ...
        for indicator in indicators {
            if itemDataSource.isEmpty {
                ...
                let indicatorParams = HCYSegmentedIndicatorSelectedParams(currentSelectedIndex: selectedIndex,
    ...
    open func reloadItem(at index: Int) {
        ...
        let cell = collectionView.cellForItem(at: IndexPath(item: index, section: 0)) as? HCYSegmentedBaseCell
    ...
    open override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        if keyPath == "contentOffset" {
            ...
            if contentScrollView?.isTracking == true || contentScrollView?.isDecelerating == true {
                ...
                let indicatorParams = HCYSegmentedIndicatorTransitionParams(currentSelectedIndex: selectedIndex,
                                                                           leftIndex: baseIndex,
                ...
                if remainderProgress == 0 {
                    ...
                    let leftCell = collectionView.cellForItem(at: IndexPath(item: baseIndex, section: 0)) as? HCYSegmentedBaseCell
                    leftCell?.reloadData(itemModel: itemDataSource[baseIndex], selectedType: .unknown)

                    let rightCell = collectionView.cellForItem(at: IndexPath(item: baseIndex + 1, section: 0)) as? HCYSegmentedBaseCell
    ...
    private func selectItemAt(index: Int, selectedType: HCYSegmentedViewItemSelectedType) {
        guard index >= 0 && index < itemDataSource.count else {
            return
        }
        ...
        let currentSelectedCell = collectionView.cellForItem(at: IndexPath(item: selectedIndex, section: 0)) as? HCYSegmentedBaseCell
        currentSelectedCell?.reloadData(itemModel: currentSelectedItemModel, selectedType: selectedType)

        let willSelectedCell = collectionView.cellForItem(at: IndexPath(item: index, section: 0)) as? HCYSegmentedBaseCell
        ...
        if scrollingTargetIndex != -1 && scrollingTargetIndex != index {
            ...
            let scrollingTargetCell = collectionView.cellForItem(at: IndexPath(item: scrollingTargetIndex, section: 0)) as? HCYSegmentedBaseCell
...
extension HCYSegmentedView: UICollectionViewDataSource {
...
extension HCYSegmentedView: UICollectionViewDelegate {
...
extension HCYSegmentedView: UICollectionViewDelegateFlowLayout {
