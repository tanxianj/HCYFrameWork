...
public enum HCYSegmentedIndicatorLineStyle {
    case normal
    case lengthen
    case lengthenOffset
}

open class HCYSegmentedIndicatorLineView: HCYSegmentedIndicatorBaseView {
    open var lineStyle: HCYSegmentedIndicatorLineStyle = .normal
    ...
    open override func refreshIndicatorState(model: HCYSegmentedIndicatorSelectedParams) {
    ...
    open override func contentScrollViewDidScroll(model: HCYSegmentedIndicatorTransitionParams) {
        ...
        switch lineStyle {
        case .normal:
            targetX = HCYSegmentedViewTool.interpolate(from: leftX, to: rightX, percent: CGFloat(percent))
            if indicatorWidth == HCYSegmentedViewAutomaticDimension {
                targetWidth = HCYSegmentedViewTool.interpolate(from: leftWidth, to: rightWidth, percent: CGFloat(percent))
            }
        case .lengthen:
            ...
            if percent <= 0.5 {
                targetX = leftX
                targetWidth = HCYSegmentedViewTool.interpolate(from: leftWidth, to: maxWidth, percent: CGFloat(percent*2))
            }else {
                targetX = HCYSegmentedViewTool.interpolate(from: leftX, to: rightX, percent: CGFloat((percent - 0.5)*2))
                targetWidth = HCYSegmentedViewTool.interpolate(from: maxWidth, to: rightWidth, percent: CGFloat((percent - 0.5)*2))
            }
        case .lengthenOffset:
            //前50%，增加width，并少量移动x；后50%，少量移动x并减小width
            let maxWidth = rightX - leftX + rightWidth - lineScrollOffsetX*2
            if percent <= 0.5 {
                targetX = HCYSegmentedViewTool.interpolate(from: leftX, to: leftX + lineScrollOffsetX, percent: CGFloat(percent*2))
                targetWidth = HCYSegmentedViewTool.interpolate(from: leftWidth, to: maxWidth, percent: CGFloat(percent*2))
            }else {
                targetX = HCYSegmentedViewTool.interpolate(from:leftX + lineScrollOffsetX, to: rightX, percent: CGFloat((percent - 0.5)*2))
                targetWidth = HCYSegmentedViewTool.interpolate(from: maxWidth, to: rightWidth, percent: CGFloat((percent - 0.5)*2))
            }
        }
    ...
    open override func selectItem(model: HCYSegmentedIndicatorSelectedParams) {
