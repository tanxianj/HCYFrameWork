...
public enum HCYSegmentedListContainerType {
    case scrollView
    case collectionView
}

@objc
public protocol HCYSegmentedListContainerViewListDelegate {
    /// 如果列表是VC，就返回VC.view
    /// 如果列表是View，就返回View自己
    ///
    /// - Returns: 返回列表视图
...
@objc
public protocol HCYSegmentedListContainerViewDataSource {
    /// 返回list的数量
    func numberOfLists(in listContainerView: HCYSegmentedListContainerView) -> Int

    /// 根据index初始化一个对应列表实例，需要是遵从`HCYSegmentedListContainerViewListDelegate`协议的对象。
    /// 如果列表是用自定义UIView封装的，就让自定义UIView遵从`HCYSegmentedListContainerViewListDelegate`协议，该方法返回自定义UIView即可。
    /// 如果列表是用自定义UIViewController封装的，就让自定义UIViewController遵从`HCYSegmentedListContainerViewListDelegate`协议，该方法返回自定义UIViewController即可。
    /// 注意：一定要是新生成的实例！！！
    ///
    /// - Parameters:
    ///   - listContainerView: HCYSegmentedListContainerView
    ///   - index: 目标index
    /// - Returns: 遵从HCYSegmentedListContainerViewListDelegate协议的实例
    ...
    @objc optional func listContainerView(_ listContainerView: HCYSegmentedListContainerView, canInitListAt index: Int) -> Bool

    /// 返回自定义UIScrollView或UICollectionView的Class
    /// 某些特殊情况需要自己处理UIScrollView内部逻辑。比如项目用了FDFullscreenPopGesture，需要处理手势相关代理。
    ///
    /// - Parameter listContainerView: HCYSegmentedListContainerView
    /// - Returns: 自定义UIScrollView实例
    @objc optional func scrollViewClass(in listContainerView: HCYSegmentedListContainerView) -> AnyClass
}

open class HCYSegmentedListContainerView: UIView, HCYSegmentedViewListContainer, HCYSegmentedViewRTLCompatible {
    open private(set) var type: HCYSegmentedListContainerType
    open private(set) weak var dataSource: HCYSegmentedListContainerViewDataSource?
    open private(set) var scrollView: UIScrollView!
    /// 已经加载过的列表字典。key是index，value是对应的列表
    open private(set) var validListDict = [Int:HCYSegmentedListContainerViewListDelegate]()
    ...
    private lazy var containerVC = HCYSegmentedListContainerViewController()
    private var willAppearIndex: Int = -1
    private var willDisappearIndex: Int = -1

    public init(dataSource: HCYSegmentedListContainerViewDataSource, type: HCYSegmentedListContainerType = .scrollView) {
    ...
    open func commonInit() {
        ...
        if type == .scrollView {
            ...
            collectionView.register(HCYSegmentedRTLCollectionCell.self, forCellWithReuseIdentifier: "cell")
    ...
    //MARK: - HCYSegmentedViewListContainer
...
extension HCYSegmentedListContainerView: UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
...
class HCYSegmentedListContainerViewController: UIViewController {
