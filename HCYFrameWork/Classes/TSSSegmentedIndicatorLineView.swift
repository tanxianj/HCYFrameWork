...
public enum TSSSegmentedIndicatorLineStyle {
    case normal
    case lengthen
    case lengthenOffset
}

open class TSSSegmentedIndicatorLineView: TSSSegmentedIndicatorBaseView {
    open var lineStyle: TSSSegmentedIndicatorLineStyle = .normal
    ...
    open override func refreshIndicatorState(model: TSSSegmentedIndicatorSelectedParams) {
    ...
    open override func contentScrollViewDidScroll(model: TSSSegmentedIndicatorTransitionParams) {
        ...
        switch lineStyle {
        case .normal:
            targetX = TSSSegmentedViewTool.interpolate(from: leftX, to: rightX, percent: CGFloat(percent))
            if indicatorWidth == TSSSegmentedViewAutomaticDimension {
                targetWidth = TSSSegmentedViewTool.interpolate(from: leftWidth, to: rightWidth, percent: CGFloat(percent))
            }
        case .lengthen:
            ...
            if percent <= 0.5 {
                targetX = leftX
                targetWidth = TSSSegmentedViewTool.interpolate(from: leftWidth, to: maxWidth, percent: CGFloat(percent*2))
            }else {
                targetX = TSSSegmentedViewTool.interpolate(from: leftX, to: rightX, percent: CGFloat((percent - 0.5)*2))
                targetWidth = TSSSegmentedViewTool.interpolate(from: maxWidth, to: rightWidth, percent: CGFloat((percent - 0.5)*2))
            }
        case .lengthenOffset:
            //前50%，增加width，并少量移动x；后50%，少量移动x并减小width
            let maxWidth = rightX - leftX + rightWidth - lineScrollOffsetX*2
            if percent <= 0.5 {
                targetX = TSSSegmentedViewTool.interpolate(from: leftX, to: leftX + lineScrollOffsetX, percent: CGFloat(percent*2))
                targetWidth = TSSSegmentedViewTool.interpolate(from: leftWidth, to: maxWidth, percent: CGFloat(percent*2))
            }else {
                targetX = TSSSegmentedViewTool.interpolate(from:leftX + lineScrollOffsetX, to: rightX, percent: CGFloat((percent - 0.5)*2))
                targetWidth = TSSSegmentedViewTool.interpolate(from: maxWidth, to: rightWidth, percent: CGFloat((percent - 0.5)*2))
            }
        }
    ...
    open override func selectItem(model: TSSSegmentedIndicatorSelectedParams) {
