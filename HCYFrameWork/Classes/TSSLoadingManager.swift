...
class TSSLoadingManager:UIView{
    private var loader:TSSLoadingCircle!
    ...
    class func show(){
        DispatchQueue.main.async {
            let loadmanager = TSSLoadingManager(frame: keyWindow.bounds)
    ...
    class func hidden(){
        ...
        if let view = subViews.first(where: {$0.isKind(of: TSSLoadingManager.self)}) as? TSSLoadingManager {
