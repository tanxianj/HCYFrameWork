...
//    let disposeBag = DisposeBag()
    ...
    override func viewDidLoad() {
        ...
        if let vcs = self.navigationController?.viewControllers, vcs.count > 1{
            print("vcs.count \(vcs.count)")
            addleftbutton()
        }
        
        self.view.backgroundColor = .white
        
        DebugLog("KScreenW is \(KScreenW) KScreenH is \(KScreenH)")
    ...
    /// - Returns: true / false
    func hiddenNavigationBarHidden() -> Bool {
        return false
    }
    
    /// Whether to disable sideslip return gesture, default false
    /// - Returns: true / false
    func interactivePopDisabled() -> Bool {
        return false
    }
    
    /// Add navigation left Item
    func addleftbutton()  {
        self.navigationItem.leftBarButtonItem = self.SetupNavigationItem(type: .left, imageName: "return", titleColor: .red, itemAction: nil)
    }
    /// Add navigation right Item
    func addRightButton(title:String? = nil,imageName:String? = nil,action:@escaping ()->()){
        self.navigationItem.rightBarButtonItem = self.SetupNavigationItem(type: .right, title: title, imageName: imageName, titleColor: .black, itemAction: action)
    }
    //MARK:Quick jump
    func currentVCpushTo(_ vc:UIViewController) {
...
extension BaseViewController:Initialization{
    @objc func initializeSubViews() {
...
extension UIViewController{
    static func makeVCWithXIB<T:viewModelProtocol>(viewController:T.Type,viewModel:T.ViewModelT)->T{
    ...
    static func makeVCWithSB<T:viewModelProtocol>(viewController:T.Type,viewModel:T.ViewModelT)->T{
        var vc = makeVCWithSB(viewController: viewController)
        vc.viewModel = viewModel
        return vc
    }
    static  func makeVCWithSB<T>(viewController:T.Type)->T{
