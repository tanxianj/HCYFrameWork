...
/// NotiFication Center
public class TSSNotiFicationCenter{
    
    /// 单列
    public static let shared = TSSNotiFicationCenter()
    ...
    /// callBacks
    public var callBacks = Dictionary<String,Array<(_ notiFication:TSSNotiFication)->Void>>()
    ...
    ///   - callback: action
    public func addObserver(name:String,object:AnyObject,callback:@escaping (_ notiFication:TSSNotiFication)->Void){
        if self.observer[name] != nil {
            self.observer[name]?.append(object)
            callBacks[name]?.append(callback)
        }else{
            var array = Array<AnyObject>()
            var callbackArray = Array<(_ notiFication:TSSNotiFication)->Void>()
            array.append(object)
    ...
    public func postNotiFication(notiFication:TSSNotiFication){
        let name = notiFication.name
        if  let arry = self.observer[name]{
            for i in 0..<arry.count {
                
                let notifi = TSSNotiFication(name: notiFication.name, data: notiFication.data)
