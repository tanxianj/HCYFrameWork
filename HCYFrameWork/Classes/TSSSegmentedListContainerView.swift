...
public enum TSSSegmentedListContainerType {
    case scrollView
    case collectionView
}

@objc
public protocol TSSSegmentedListContainerViewListDelegate {
    /// 如果列表是VC，就返回VC.view
    /// 如果列表是View，就返回View自己
    ///
    /// - Returns: 返回列表视图
...
@objc
public protocol TSSSegmentedListContainerViewDataSource {
    /// 返回list的数量
    func numberOfLists(in listContainerView: TSSSegmentedListContainerView) -> Int

    /// 根据index初始化一个对应列表实例，需要是遵从`TSSSegmentedListContainerViewListDelegate`协议的对象。
    /// 如果列表是用自定义UIView封装的，就让自定义UIView遵从`TSSSegmentedListContainerViewListDelegate`协议，该方法返回自定义UIView即可。
    /// 如果列表是用自定义UIViewController封装的，就让自定义UIViewController遵从`TSSSegmentedListContainerViewListDelegate`协议，该方法返回自定义UIViewController即可。
    /// 注意：一定要是新生成的实例！！！
    ///
    /// - Parameters:
    ///   - listContainerView: TSSSegmentedListContainerView
    ///   - index: 目标index
    /// - Returns: 遵从TSSSegmentedListContainerViewListDelegate协议的实例
    ...
    @objc optional func scrollViewClass(in listContainerView: TSSSegmentedListContainerView) -> AnyClass
}

open class TSSSegmentedListContainerView: UIView, TSSSegmentedViewListContainer, TSSSegmentedViewRTLCompatible {
    open private(set) var type: TSSSegmentedListContainerType
    open private(set) weak var dataSource: TSSSegmentedListContainerViewDataSource?
    open private(set) var scrollView: UIScrollView!
    /// 已经加载过的列表字典。key是index，value是对应的列表
    open private(set) var validListDict = [Int:TSSSegmentedListContainerViewListDelegate]()
    ...
    private lazy var containerVC = TSSSegmentedListContainerViewController()
    private var willAppearIndex: Int = -1
    private var willDisappearIndex: Int = -1

    public init(dataSource: TSSSegmentedListContainerViewDataSource, type: TSSSegmentedListContainerType = .scrollView) {
    ...
    open func commonInit() {
        ...
        if type == .scrollView {
            ...
            collectionView.register(TSSSegmentedRTLCollectionCell.self, forCellWithReuseIdentifier: "cell")
    ...
    //MARK: - TSSSegmentedViewListContainer
...
extension TSSSegmentedListContainerView: UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
...
class TSSSegmentedListContainerViewController: UIViewController {
